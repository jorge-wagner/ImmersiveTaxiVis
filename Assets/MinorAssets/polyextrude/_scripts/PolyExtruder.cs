...
public class PolyExtruder : MonoBehaviour
{
    ...
    private float customBottomY = 0.0f;
    private float customTopY = 1.0f;
    ...
    public void createPrism(string prismName, float height, Vector2[] vertices, Color32 color, bool is3D, bool isUsingBottomMeshIn3D, bool isUsingColliders)
    {
        ...
        this.customTopY = this.customBottomY + height; 
    ...
    public void createPrism(string prismName, float height, float customBottomY, Vector2[] vertices, Color32 color, bool is3D, bool isUsingBottomMeshIn3D, bool isUsingColliders)
    {
        this.customBottomY = customBottomY;
        createPrism(prismName, height, vertices, color, is3D, isUsingBottomMeshIn3D, isUsingColliders);
    }
    ...
    private void initPrism()
    {
        ...
        try
        { 
            Triangulation.triangulate(pointsB, holesB, customBottomY, out indicesB, out verticesB);
        }
        catch 
        {
            Destroy(goB);
        ...
        */
        ...
        if(this.is3D)
        {
			...
			// perform TRIANGULATION
			Triangulation.triangulate(pointsT, holesT, customTopY, out indicesT, out verticesT);
			...
			*/
			...
			{
				verticesS.Add(new Vector3(bv.x, customBottomY, bv.y));
			}
			foreach (Vector2 tv in pointsT)
			{
				verticesS.Add(new Vector3(tv.x, customTopY, tv.y));
			}
    ...
    {
        //this.gameObject.transform.localPosition = new Vector3(this.polygonCentroid.x, customBottomY, this.polygonCentroid.y);
        this.gameObject.transform.position = new Vector3(this.polygonCentroid.x, customBottomY, this.polygonCentroid.y);
    }
    ...
    public bool initOutlineRenderer()
    {
        ...
        float yPos = customTopY;
        if (!this.is3D) yPos = customBottomY;
