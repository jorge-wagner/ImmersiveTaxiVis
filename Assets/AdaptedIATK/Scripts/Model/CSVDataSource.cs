...
namespace IATK
{
    ...
    public class CSVDataSource : DataSource
    {
        ...
        CultureInfo date_us = new CultureInfo("en-US");
        public bool inverseTime = false;
        

        string date_format = "yyyy-MM-dd H:mm:ss";
        //DateTime base_date = DateTime.ParseExact("2000-01-01 0:00:00", "yyyy-MM-dd H:mm:ss", new CultureInfo("en-US"));
        DateTime base_date = DateTime.ParseExact("2010-01-01 0:00:00", "yyyy-MM-dd H:mm:ss", new CultureInfo("en-US"));
        DateTime base_future_date = DateTime.ParseExact("2030-01-01 0:00:00", "yyyy-MM-dd H:mm:ss", new CultureInfo("en-US"));
        ...
        public override object getOriginalValue(float normalisedValue, string identifier)
        {
            ...
            else if (meta.type == DataType.DateTime)
            {
                normValue = normaliseValue(valueClosestTo(this[identifier].Data, normalisedValue), 0f, 1f, meta.minValue, meta.maxValue);
                //return base_date.AddSeconds(normValue);
                if(inverseTime)
                    return base_future_date.AddSeconds(-normValue);
                else
                    return base_date.AddSeconds(normValue);
                //return DateTime.FromOADate(normValue);
            }
            else return normValue;
        }
                                    ...
                                    case DataType.DateTime:
                                        {
                                            ...
                                            //string format = "yyyy-MM-dd H:mm:ss";                           // TO DO: MAKE MORE ROBUST
                                            //Debug.Log(cleanedValue);
                                            DateTime t = DateTime.ParseExact(cleanedValue, date_format, date_us);

                                            if(inverseTime) 
                                                dataArray[i - 1, k] = (float)(base_future_date.Subtract(t).TotalSeconds);   //((float)t.ToOADate());
                                            else
                                                dataArray[i - 1, k] = (float)t.Subtract(base_date).TotalSeconds;   //((float)t.ToOADate());
                                            //dataArray[i - 1, k] = ((float)t.ToOADate());
                                            //Debug.Log(dataArray[i - 1, k]);
