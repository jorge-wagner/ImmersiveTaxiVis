...
namespace IATK
{
    ...
    public class LinkingVisualisations : MonoBehaviour
    {
     ...
     ///  Adapted by jawfilho
     /// </summary>
    public class LinkingViews : MonoBehaviour
    {
        ...
        public View visualisationSource = null;
        public View visualisationTarget = null;
        ...
        void OnEnable()
        {
            ...
            linkerMaterial = Instantiate(linkerMaterial);


            if (visualisationSource != null && visualisationTarget != null) LinkViews();
            toggleShow = showLinks;
        ...
        void Update()
        {
            ...
            if (view != null && showLinks)                              
            {
                ...
                linkerMaterial.SetFloat("_MinXFilter1", visualisationSource.GetMinX());
                linkerMaterial.SetFloat("_MinXFilter2", visualisationTarget.GetMinX());
                linkerMaterial.SetFloat("_MinYFilter1", visualisationSource.GetMinY());
                linkerMaterial.SetFloat("_MinYFilter2", visualisationTarget.GetMinY());
                linkerMaterial.SetFloat("_MinZFilter1", visualisationSource.GetMinZ());
                linkerMaterial.SetFloat("_MinZFilter2", visualisationTarget.GetMinZ());

                linkerMaterial.SetFloat("_MaxXFilter1", visualisationSource.GetMaxX());
                linkerMaterial.SetFloat("_MaxXFilter2", visualisationTarget.GetMaxX());
                linkerMaterial.SetFloat("_MaxYFilter1", visualisationSource.GetMaxY());
                linkerMaterial.SetFloat("_MaxYFilter2", visualisationTarget.GetMaxY());
                linkerMaterial.SetFloat("_MaxZFilter1", visualisationSource.GetMaxZ());
                linkerMaterial.SetFloat("_MaxZFilter2", visualisationTarget.GetMaxZ());

                linkerMaterial.SetFloat("_MinNormX1", visualisationSource.GetMinNormX());
                linkerMaterial.SetFloat("_MinNormX2", visualisationTarget.GetMinNormX());
                linkerMaterial.SetFloat("_MinNormY1", visualisationSource.GetMinNormY());
                linkerMaterial.SetFloat("_MinNormY2", visualisationTarget.GetMinNormY());
                linkerMaterial.SetFloat("_MinNormZ1", visualisationSource.GetMinNormZ());
                linkerMaterial.SetFloat("_MinNormZ2", visualisationTarget.GetMinNormZ());

                linkerMaterial.SetFloat("_MaxNormX1", visualisationSource.GetMaxNormX());
                linkerMaterial.SetFloat("_MaxNormX2", visualisationTarget.GetMaxNormX());
                linkerMaterial.SetFloat("_MaxNormY1", visualisationSource.GetMaxNormY());
                linkerMaterial.SetFloat("_MaxNormY2", visualisationTarget.GetMaxNormY());
                linkerMaterial.SetFloat("_MaxNormZ1", visualisationSource.GetMaxNormZ());
                linkerMaterial.SetFloat("_MaxNormZ2", visualisationTarget.GetMaxNormZ());


                linkerMaterial.SetFloat("_Alpha", linkTransparency);

                // Set scaling matrices based on visualisation size
                //linkerMaterial.SetMatrix("_ScaleMatrix1", Matrix4x4.TRS(Vector3.zero, Quaternion.identity, this.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix2", Matrix4x4.TRS(Vector3.zero, Quaternion.identity, this.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix1", Matrix4x4.TRS(Vector3.zero, Quaternion.identity, visualisationSource.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix2", Matrix4x4.TRS(Vector3.zero, Quaternion.identity, visualisationTarget.transform.localScale));                
                //linkerMaterial.SetMatrix("_ScaleMatrix1", Matrix4x4.TRS(Vector3.zero, Quaternion.identity, visualisationSource.transform.lossyScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix2", Matrix4x4.TRS(Vector3.zero, Quaternion.identity, visualisationTarget.transform.lossyScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix1", Matrix4x4.TRS(this.transform.position, this.transform.rotation, this.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix2", Matrix4x4.TRS(this.transform.position, this.transform.rotation, this.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix1", Matrix4x4.TRS(this.transform.position, this.transform.rotation, visualisationSource.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix2", Matrix4x4.TRS(this.transform.position, this.transform.rotation, visualisationTarget.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix1", Matrix4x4.TRS(visualisationSource.transform.position, visualisationSource.transform.rotation, visualisationSource.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix2", Matrix4x4.TRS(visualisationTarget.transform.position, visualisationTarget.transform.rotation, visualisationTarget.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix1", Matrix4x4.TRS(visualisationSource.transform.localPosition, visualisationSource.transform.localRotation, visualisationSource.transform.localScale));
                //linkerMaterial.SetMatrix("_ScaleMatrix2", Matrix4x4.TRS(visualisationTarget.transform.localPosition, visualisationTarget.transform.localRotation, visualisationTarget.transform.localScale));
                linkerMaterial.SetMatrix("_ScaleMatrix1", Matrix4x4.TRS(Vector3.zero, Quaternion.identity, Vector3.one));
                linkerMaterial.SetMatrix("_ScaleMatrix2", Matrix4x4.TRS(Vector3.zero, Quaternion.identity, Vector3.one));

                view.transform.localPosition = new Vector3(.5f, .5f, .5f);
        ...
        void CalculateCorners(View v,  ref Vector3 ftl, ref Vector3 ftr, ref Vector3 fbl, ref Vector3 fbr,
            ref Vector3 btl, ref Vector3 btr, ref Vector3 bbl, ref Vector3 bbr)
        {

            //CalculateCorners3(v.transform, ref ftl, ref ftr, ref fbl, ref fbr, ref btl, ref btr, ref bbl, ref bbr);


            //Vector3 up = transform.TransformVector(transform.up);
            //Vector3 down = transform.TransformVector(-transform.up);
            //Vector3 forward = transform.TransformVector(transform.forward);
            //Vector3 back = transform.TransformVector(-transform.forward);
            //Vector3 left = transform.TransformVector(-transform.right);
            //Vector3 right = transform.TransformVector(transform.right);

            Vector3 up = transform.up / 2;
            Vector3 down = -transform.up / 2; 
            Vector3 forward = transform.forward / 2;
            Vector3 back = -transform.forward / 2;
            Vector3 left = -transform.right / 2;
            Vector3 right = transform.right / 2;


            ftl = forward + right + down;
            ftr = forward + right + up;
            fbl = forward + left + down;
            fbr = forward + left + up;
            ...
            ftl = v.transform.TransformPoint(((ftl)));
            ftr = v.transform.TransformPoint(((ftr)));
            fbl = v.transform.TransformPoint(((fbl)));
            fbr = v.transform.TransformPoint(((fbr)));

            btl = v.transform.TransformPoint(((btl)));
            btr = v.transform.TransformPoint(((btr)));
            bbl = v.transform.TransformPoint(((bbl)));
            bbr = v.transform.TransformPoint(((bbr)));

            //ftl = transform.TransformPoint(transform.InverseTransformVector((ftl)));
            //ftr = transform.TransformPoint(transform.InverseTransformVector((ftr)));
            //fbl = transform.TransformPoint(transform.InverseTransformVector((fbl)));
            //fbr = transform.TransformPoint(transform.InverseTransformVector((fbr)));

            //btl = transform.TransformPoint(transform.InverseTransformVector((btl)));
            //btr = transform.TransformPoint(transform.InverseTransformVector((btr)));
            //bbl = transform.TransformPoint(transform.InverseTransformVector((bbl)));
            //bbr = transform.TransformPoint(transform.InverseTransformVector((bbr)));
        ...
        public void LinkViews()
        {
            ...
            Vector3[] vSource = visualisationSource.GetPositions();// bigMesh.getBigMeshVertices();
            Vector3[] vDest = visualisationTarget.GetPositions();// bigMesh.getBigMeshVertices();
            List<Color> colors = new List<Color>();
            Color[] cSource = visualisationSource.GetColors();// bigMesh.getColors();
            Color[] cDest = visualisationTarget.GetColors();// bigMesh.getColors();
            ...
            viewLinked = new ViewBuilder(MeshTopology.Lines, gameObject.name)
            .initialiseDataView(positionsLocalX.Count)
            .setDataDimension(positionsLocalX.ToArray(), ViewBuilder.VIEW_DIMENSION.X)
            .setDataDimension(positionsLocalY.ToArray(), ViewBuilder.VIEW_DIMENSION.Y)
            .setDataDimension(positionsLocalZ.ToArray(), ViewBuilder.VIEW_DIMENSION.Z);

            //viewLinked = viewLinked.createIndicesPointTopology();

            viewLinked.Indices = indexBuffer;
            viewLinked.updateView();
            view = viewLinked.apply(this.gameObject, linkerMaterial, "Links");
            view.SetColors(colors.ToArray());
            view.SetAllChannels(normalsBuffer.ToArray());
