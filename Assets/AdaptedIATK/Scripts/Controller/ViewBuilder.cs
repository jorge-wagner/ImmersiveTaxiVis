...
namespace IATK
{
    ...
    public class ViewBuilder
    {
        ...
        private bool duplicatedPoints = false; // jwagner
        ...
        public ViewBuilder initialiseDataView(int numberOfPoints, bool duplicatePoints)
        {
            if (!duplicatePoints)
                return initialiseDataView(numberOfPoints);

            // Initialise
            positions = new List<Vector3>(2 * numberOfPoints);
            Indices = new List<int>();
            colours = new List<Color>(2* numberOfPoints);
            normals = new List<Vector3>(2 * numberOfPoints);
            uvs = new List<Vector3>(2 * numberOfPoints);
            uv2s = new List<Vector3>(2 * numberOfPoints);

            LineLength = new List<int>();
            chunkSizeList = new List<int>();

            numberOfDataPoints = 2 * numberOfPoints;
        ...
        /// <param name="data">Data.</param>
        /// <param name="dimension">Dimension.</param>
        public ViewBuilder setDataDimension(float[] data, VIEW_DIMENSION dimension, bool duplicatePoints)
        {
            if (!duplicatePoints)
                return setDataDimension(data, dimension);
        ...
        {
            colours = new List<Color>(numberOfDataPoints); // jawfilho
        ...
        public View apply(GameObject parent, Material material, string name)
        {
            parent.name = Name;
        ...
        public View applyCopy(GameObject parent, View originalView, Material material, string name)
        {
            //parent.name = Name;
            ...
            //BigMesh meshObject = originalView.BigMesh;// BigMesh.createBigMesh(data);
            //meshObject.gameObject.name = "BigMesh";
            //meshObject.transform.SetParent(viewGameObject.transform, false);

            //bigMesh = meshObject;
            view.BigMesh = originalView.BigMesh;// bigMesh;
