Shader "IATK/SphereShaderEdited"
{
	Properties
	{
		...
		SubShader
		{
			...
			void GS_Main(point v2g p[1], inout TriangleStream<g2f> triStream)
			{
				...
				triStream.Append(o);*/
					...
					v[0].pos = UnityObjectToClipPos(float4(p[0].pos + halfS * right - halfS * up, 1.0f)); //
					v[0].normal = p[0].normal; //
					v[0].color = p[0].color; // WAS 0
					v[0].isBrushed = p[0].isBrushed; // WAS 0 // || points[1].isBrushed;

					v[1].tex0 = float2(1.0f, 0.0f);
					v[1].pos = UnityObjectToClipPos(float4(p[0].pos + halfS * right + halfS * up, 1.0f)); //
					v[1].normal = p[0].normal; //
					v[1].color = p[0].color; // WAS 0
					v[1].isBrushed = p[0].isBrushed; // WAS 0 // || points[1].isBrushed;

					v[2].tex0 = float2(1.0f, 0.0f);
					v[2].pos = UnityObjectToClipPos(float4(p[0].pos - halfS * right - halfS * up, 1.0f)); //
					v[2].normal = p[0].normal; //
					v[2].color = p[0].color; // WAS 0
					v[2].isBrushed = p[0].isBrushed; // WAS 0 // || points[1].isBrushed;

					v[3].tex0 = float2(1.0f, 0.0f);
					v[3].pos = UnityObjectToClipPos(float4(p[0].pos - halfS * right + halfS * up, 1.0f)); //
					v[3].normal = p[0].normal; //
					v[3].color = p[0].color; // WAS 0
					v[3].isBrushed = p[0].isBrushed; // WAS 0 // || points[1].isBrushed;

		

					UNITY_TRANSFER_VERTEX_OUTPUT_STEREO(p[0], v[0]);
					triStream.Append(v[0]);
					UNITY_TRANSFER_VERTEX_OUTPUT_STEREO(p[0], v[1]);
					triStream.Append(v[1]);
					UNITY_TRANSFER_VERTEX_OUTPUT_STEREO(p[0], v[2]);
					triStream.Append(v[2]);
					UNITY_TRANSFER_VERTEX_OUTPUT_STEREO(p[0], v[3]);
			...
			f_output FS_Main(g2f input)
			{
				...
				*/
				...
				if (input.color.w == 0 || (!input.isBrushed && BrushAsFilter > 0.0))
					{
						discard;
					...
					else if (input.isBrushed && ShowBrush > 0.0)
					{
						o.color = BrushColor;
					}
					else
