Shader "IATK/SphereShaderEdited2"
{
	Properties
	{
	...
	_ODBrushedTexture("OD Brush Texture", 2D) = "white" {}
		...
		Pass
		{
			...
			CGPROGRAM
				...
				struct v2g
				{
					...
					//float isODBrushed : FLOAT;
					//float isHighlighted : FLOAT;
					float3 isBrushed : TEXCOORD0;
					float3 normal : NORMAL;
				...
				UNITY_INSTANCING_BUFFER_START(Props)
					...
					UNITY_DEFINE_INSTANCED_PROP(float, _IndependentBrushing)
			...
			sampler2D _ODBrushedTexture;
				...
				v2g vert(appdata v)
				{
					...
					float4 ODbrushedValue = tex2Dlod(_ODBrushedTexture, float4(indexUV, 0.0, 0.0));
					//half3 brushValue = tex2D(_BrushedTexture, indexUV);
					float isODBrushed = ODbrushedValue.r;
				...
				//o.isHighlighted = p[0].isHighlighted;
				//o.isODBrushed = p[0].isODBrushed;
				...
				f_output frag(g2f i)
				{
					...
					float IndependentBrushing = UNITY_ACCESS_INSTANCED_PROP(Props, _IndependentBrushing);
					...
					half2 d = i.tex0 - float2(0.5, 0.5);
						...
						if ((length(d) > 0.5 || i.color.w == 0 || (!i.isBrushed.x && BrushAsFilter > 0.0) || !i.isBrushed.y))
						{
							discard;
						...
						else if (i.isBrushed.x && i.isBrushed.z)
						{
							if (i.isBrushed.z) //isHighlighted
							{
								o.color = _LightColor0;
								o.color.a = i.color.a;
								o.color.rgb *= nl;
								o.color.rgb += ShadeSH9(half4(worldNormal, 1));
								//o.color.rgb *= float4(i.color.x + 0.2, i.color.y + 0.2, i.color.z + 0.2, i.color.w);
								//if(i.color.r == 1.0)
								//	o.color.rgb *= float4(i.color.r, i.color.g + 0.5, i.color.b + 0.5, i.color.a);
								//else(i.color.b == 1.0)
								//	o.color.rgb *= float4(i.color.r + 0.5, i.color.g + 0.5, i.color.b, i.color.a);
								//else 
								o.color.rgb *= float4(i.color.r * 0.2, i.color.g * 0.2, i.color.b * 0.2, i.color.a);

								//o.color.rgb *= float4(0,1,0,0);
						...
						else if (i.isBrushed.x && ShowBrush > 0.0)
						{
							o.color = _LightColor0;
							o.color.a = i.color.a;
							o.color.rgb *= nl;
