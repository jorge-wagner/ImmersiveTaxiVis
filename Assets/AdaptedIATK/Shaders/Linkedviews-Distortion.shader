		...
		Pass
		{
			...
			CGPROGRAM
				...
				v2g vert(appdata v)
				{
					...
					o.isBrushed = float3(isBrushed, isODBrushed, isHighlighted);
					...
					//o.vertex = mul(UNITY_MATRIX_VP, ObjectToWorldDistort(pos));
					//o.vertex = ObjectToWorldDistort3d(pos, v.normal.z > 0);
					//o.vertex = UnityObjectToClipPos(ObjectToWorldDistort3d(pos, v.normal.z > 0));
					//o.vertex = mul(unity_ObjectToWorld, ObjectToWorldDistort3d(pos, v.normal.z > 0));
					//o.vertex = mul(UNITY_MATRIX_VP, ObjectToWorldDistort3d(pos,false));
				...
				{					
					...
					if (!filtered)
					{
						...
						o.color = l[0].color;
						o.vertex = l[0].vertex; 
						//o.vertex = UnityObjectToClipPos(l[0].vertex);
						o.uv = l[0].uv;
						...
						o.color = l[1].color;
						o.vertex = l[1].vertex;
						//o.vertex = UnityObjectToClipPos(l[1].vertex);
						o.uv = l[1].uv;
						o.isBrushed = l[1].isBrushed;
						UNITY_TRANSFER_VERTEX_OUTPUT_STEREO(l[0], o);
						lineStream.Append(o);

						//l[0].color = float4(1,1,1,1);
						//l[1].color = float4(1, 1, 1, 1);
				...
				f_output frag(g2f i)
				{
					...
					//if ((!i.isBrushed.x && BrushAsFilter > 0.0) || !i.isBrushed.y)
					if (i.color.w == 0 || (Alpha < 0.01) || (!i.isBrushed.x && BrushAsFilter > 0.0) || !i.isBrushed.y)
					//if (length(d) > 0.5 || i.color.w == 0 || (Alpha < 0.01) || (!i.isBrushed.x && BrushAsFilter > 0.0) || !i.isBrushed.y)
				//if (i.color.w == 0 || (Alpha < 0.01) || (!i.isBrushed.x && BrushAsFilter > 0.0) || !i.isBrushed.y)
					{
						discard;
					...
					/*
					if (i.isBrushed.x == 0.0 && BrushAsFilter > 0.0 && i.isBrushed.y == 0.0)
					{
						discard;
					}
					else if (i.isBrushed.x > 0.0 && ShowBrush > 0.0)
					{
						o.color = float4(BrushColor.x, BrushColor.y, BrushColor.z, Alpha);
					}
					else if (Alpha < 0.01)
