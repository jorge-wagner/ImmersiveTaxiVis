Shader "IATK/OutlineDots2DEditedVertical" 
{    Properties
    {
		_MainTex("Texture", 2D) = "white" {}
		_BrushedTexture("Brush as Filter Texture", 2D) = "white" {}
		_ODBrushedTexture("OD Brushing Texture", 2D) = "white" {}
		_HighlightTexture("Highlighting Texture", 2D) = "white" {}		
        ...
        //_Color("_Color", Color) = (1,1,1,1) // jawfilho
    }
    ...
    SubShader
    {
        Pass
        {
			...
			Blend [_MySrcMode][_MyDstMode] // One One
			Cull Off 
			Lighting Off 
			LOD 200
			Offset -1, -1
            ...
            CGPROGRAM
                ...
                //#pragma noshadow  // jawfilho
                //#pragma nodynlightmap  // jawfilho
                //#pragma noforwardadd // jawfilho
                ...
                v2g vert(app_data v)
                {
					...
					float isBrushed = brushValue.r;
					float4 odBrushValue = tex2Dlod(_ODBrushedTexture, float4(indexUV, 0.0, 0.0));
					float isODBrushed = odBrushValue.r;
					float4 highlightValue = tex2Dlod(_HighlightTexture, float4(indexUV, 0.0, 0.0));
					float isHighlighted = highlightValue.r;

					output.isBrushed = float3(isBrushed, isODBrushed, isHighlighted);
                ...
                void geom(point v2g input[1], inout TriangleStream<g2f> triStream)
                {
                    ...
                    //float3 objectUp = unity_WorldToObject[2].xyz;   // modified by jawfilho //2
                    //float3 objectRight = unity_WorldToObject[0].xyz; // modified by jawfilho //0

					float3 objectUp = mul(float3(0, 1, 0), unity_WorldToObject);
					float3 objectForward = mul(float3(0, 0, 1), unity_WorldToObject);
                    ...
                    //v[0] = float4(input[0].pos + halfS * float3(0, 1, 0) - halfS * float3(0, 0, 1), 1.0f); // modified by jawfilho
                    //v[1] = float4(input[0].pos + halfS * float3(0, 1, 0) + halfS * float3(0, 0, 1), 1.0f); // modified by jawfilho
                    //v[2] = float4(input[0].pos - halfS * float3(0, 1, 0) - halfS * float3(0, 0, 1), 1.0f); // modified by jawfilho
                    //v[3] = float4(input[0].pos - halfS * float3(0, 1, 0) + halfS * float3(0, 0, 1), 1.0f); // modified by jawfilho

					v[0] = float4(input[0].pos + halfS * objectUp - halfS * objectForward, 1.0f); // modified by jawfilho
					v[1] = float4(input[0].pos + halfS * objectUp + halfS * objectForward, 1.0f); // modified by jawfilho
					v[2] = float4(input[0].pos - halfS * objectUp - halfS * objectForward, 1.0f); // modified by jawfilho
					v[3] = float4(input[0].pos - halfS * objectUp + halfS * objectForward, 1.0f); // modified by jawfilho
                ...
                f_output frag(g2f input)
                {
					...
					//if (length(d) > 0.5 || input.color.w == 0 || (!input.isBrushed.x && BrushAsFilter > 0.0) || !input.isBrushed.y)
					if (input.color.w == 0 || (!input.isBrushed.x && BrushAsFilter > 0.0) || !input.isBrushed.y)
					{
						discard;
					...
					else if (input.isBrushed.x > 0 && ShowBrush > 0.0)
						output.color = tex2D(_MainTex, input.tex0.xy) * BrushColor;
					//else if (input.isBrushed.y > 0)
					//	output.color = tex2D(_MainTex, input.tex0.xy) * input.color;
					else if (input.isBrushed.z) //isHighlighted
						output.color = tex2D(_MainTex, input.tex0.xy) * float4(input.color.r * 0.2, input.color.g * 0.2, input.color.b * 0.2, input.color.a);
					else
						output.color = tex2D(_MainTex, input.tex0.xy) * input.color;
